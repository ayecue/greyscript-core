...
export class UnexpectedParameterInFunction extends Error {
  ...
  constructor(token: Token) {
    super(
      `Unexpected parameter in function declaration at line ${token.line}.`
    );
    const me = this;
...
export class UnexpectedNonStringLiteralInImportCode extends Error {
  ...
  constructor(token: Token) {
    super(
      `Unexpected import path at ${token.line}. Import code only allows a hardcoded import path.`
    );
    const me = this;
...
export class CallExpressionEOL extends Error {
  ...
  constructor(token: Token, previousToken: Token) {
    super(
      `Call expressions do not support multiline arguments. Discrepancy found between ${previousToken.line} and ${token.line}.`
    );
    const me = this;
